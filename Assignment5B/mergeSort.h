...
int max_length = 0;
int current_length_in_tapes[MAX];
...
void put_songs(song song_list[],int n_of_songs,song tapes[][MAX],int n_of_tapes){
    ...
    for(int i = 0; i < n_of_songs; i++){
        if((song_list[i].length + current_length_in_tapes[i % n_of_tapes]) > max_length){
            ...
            current_length_in_tapes[i%n_of_tapes] += song_list[i].length;
        }
    }
}

void show_tapes(song song_list[],int n_of_songs,song tapes[][MAX],int n_of_tapes){
    int j=0;
    int MRT = 0;
